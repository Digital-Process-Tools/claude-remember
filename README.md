...
**Gemini CLI sets `CLAUDE_PROJECT_DIR`, per the same bundled docs -- and this repo's Codex section above, `scripts/resolve-paths.sh`, `scripts/lib-env-cache.sh`, and three of the hook scripts all assumed until now that it never does** ([#534](https://github.com/Digital-Process-Tools/claude-remember/issues/534)). `pipeline/host.py`'s own `GEMINI` host is corrected in this change (`project_dir_vars=("CLAUDE_PROJECT_DIR",)`, `tests/test_gemini_project_dir_var_456.py`) since that correction is narrow, dead-code-only, and inside this diff's own file; the wider shell-script and README assumption is not touched here and is filed as #534, since whether Gemini CLI's `UserPromptSubmit`/`BeforeAgent` stdout-contract branch (which keys on this exact variable) still does the right thing once `CLAUDE_PROJECT_DIR` is actually set needs a live session to settle, not more documentation reading.
