...
- **Trusted by design.** A manual `scripts/save-session.sh` run, `scripts/doctor.sh`, and a direct `pipeline.extract` invocation from the command line have no hook preamble to clear anything in, and inherit the ambient environment on purpose — the same way they already inherit `$PATH` or `$HOME`. A containment rule that does not need host-specific knowledge was considered and rejected: a real transcript lives under `CLAUDE_CONFIG_DIR`/`~/.claude`, never under the project it describes, so "must be under the project directory" would reject legitimate paths, and "must be under the known session store" only has a known store because this repo happens to compute one for Claude Code — encoding that as a gate is exactly the host-specific seam `pipeline/host.py`'s own module docstring declines to invent. `scripts/doctor.sh` says so loudly: it prints a `WARN` naming the value when it finds one in its own environment, rather than silently trusting it. See `pipeline/host.transcript_path()`'s docstring for the fuller argument.
