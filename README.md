...
| `REMEMBER_SUMMARIZER` | Which summarizer provider to use: `claude` (always `claude -p`, the historical behaviour), `codex` (always `codex exec`), or `auto` (the default) — `auto` asks `pipeline.host.detect_host()` and follows it: `codex` under a detected Codex host, `claude` everywhere else (Claude Code, an unrecognised host, or no signature at all). A Claude Code session's own resolution is unchanged by this: it was already `claude`, and `auto` still answers `claude` for it. An unrecognised value is reported and treated as unset ([#460](https://github.com/Digital-Process-Tools/claude-remember/issues/460)). |
