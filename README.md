...
The `SessionEnd` check only counts a quiet transcript as evidence once it is newer than `.remember/.gitignore`'s own mtime -- written exactly once, the first time any hook bootstraps the store, and never touched again -- since a transcript quiet since before that moment cannot be proof `SessionEnd` failed to fire: the hook was never registered for it. Installing into (or upgrading in) a project with pre-existing Claude Code history therefore reads correctly as "nothing has had the chance to prove or disprove this yet", not as a false `problem`. In external storage mode, where `.gitignore` is not written into the store, this check can currently only `WARN`, never `FAIL` ([#392](https://github.com/Digital-Process-Tools/claude-remember/issues/392)).
