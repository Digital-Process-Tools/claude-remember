...
**This is an existence check only, not a containment check** ([#424](https://github.com/Digital-Process-Tools/claude-remember/issues/424)): `pipeline/extract.py`'s `find_session()` returns whatever `transcript_path()` hands it before its own traversal validation ever runs, so a `REMEMBER_TRANSCRIPT_PATH` set anywhere in the ambient environment -- not written by either hook -- is trusted unconditionally. `scripts/post-tool-hook.sh` and `scripts/user-prompt-hook.sh`, the two hooks that trigger routine autosaves and have no `transcript_path` of their own to offer, now `unset REMEMBER_TRANSCRIPT_PATH` before doing anything else. Consolidation, `/remember:doctor`, and a manual `pipeline.extract` invocation read whatever their own process environment already holds and are not covered by that unset.
