...
**Extraction itself did not work until [#443](https://github.com/Digital-Process-Tools/claude-remember/issues/443).** Codex writes a different transcript envelope than Claude Code -- every line is `{"timestamp", "ordinal", "type", "payload"}`, with the role and text one level inside `payload` -- and `pipeline/extract.py` originally only recognised Claude Code's shape, so a real Codex session with a real human prompt and a real reply extracted `0 exchanges` and saved nothing, with every outward signal (hooks firing, `.remember/` present) suggesting it had worked. `pipeline/host.py` now sniffs a transcript's envelope from its own first line and reads either shape; an envelope matching neither is reported as `"unrecognised"` rather than silently counted as an empty session. A trimmed, sanitised capture of the reproducing session lives at `tests/fixtures/codex-rollout.jsonl`. **"First line" needed correcting once current Claude Code (2.1.257+) stopped opening transcripts with a message line at all** ([#543](https://github.com/Digital-Process-Tools/claude-remember/issues/543)): every session now opens with several bookkeeping records (`bridge-session`, `queue-operation`, `mode`, and others) that `sniff_envelope()` cannot place, ahead of the first real `user`/`assistant` line, so a strict first-line decision read every current session as `"unrecognised"` and captured nothing. `sniff_file_envelope_status()` (`pipeline/extract.py`) now scans forward past a line it cannot place -- up to `_ENVELOPE_SNIFF_SCAN_CAP` (50) such lines -- and returns the first verdict that actually resolves, falling back to `"unrecognised"` only once the file (or the cap) is exhausted without ever resolving.
