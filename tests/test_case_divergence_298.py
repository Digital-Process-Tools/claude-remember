...
_SANCTIONED_DIVERGENCE = {
    ...
    "scripts/lib-memory-dir.sh": [
        ...
        (
            ...
            # #766/#757 hardening: the tracked_status function grows a
            # git-missing disk-walk (fail closed inside a real repo,
            # migrate normally otherwise) and switches its ls-files check
            # to `:(icase)` pathspec magic so a differently-cased tracked
            # path is still seen; a symlinked project config.json is also
            # refused before anything below can read through it. Folded
            # into this SAME pair rather than stacked (#734/#746
            # precedent).
            '_project_cfg="${REMEMBER_DIR}/config.json"\n'
            '\n'
            '_classify_project_cfg_haiku_trust() {\n'
