...
2. Auto-migrates any existing `<project>/.remember/` to the new location — once, leaving a `MIGRATED-TO.txt` marker in the old directory. **Exception:** a `config.json` in that legacy directory that is tracked by git — the ENCLOSING repository's index, not just a `.git` in the project directory itself, so a project started from a repo subdirectory is still covered — is left behind, still tracked, rather than migrated ([#757](https://github.com/Digital-Process-Tools/claude-remember/issues/757)) — once external, `${REMEMBER_DIR}/config.json` is trusted outright (see [configuration.md](configuration.md)), and a config a repository committed is exactly the untrusted input [#726](https://github.com/Digital-Process-Tools/claude-remember/issues/726)/[#740](https://github.com/Digital-Process-Tools/claude-remember/issues/740) already strip out of the ordinary project layer. **When the tracked status cannot be determined at all** — a git spawn failing for a reason other than a confirmed answer — the file is left behind too: a git failure must never read the same as a confirmed absence and let the file migrate as trusted by default. The marker names why, and a `remember:` line goes to `hook-errors.log`. An **untracked** legacy `config.json` — the user's own, never committed, even in a real git repo — still migrates and is still trusted, as before. Case-insensitivity (a differently-cased `Config.json` on a case-insensitive filesystem escaping the check) is not handled — this would cost a second git spawn to rule out and was left for a follow-up rather than paid on this path.
