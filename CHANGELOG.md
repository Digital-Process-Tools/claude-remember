...
- **Re-enabled Windows shell-subprocess coverage** ([#79](https://github.com/Digital-Process-Tools/claude-remember/issues/79)) — `test_log_sh`, `test_migration`, and `test_security_fixes` were `skipif(win32)` because the runner's PATH `bash` is the WSL launcher. Two real layers fixed: tests now invoke bash by its explicit Git-for-Windows path (`subprocess.run(["bash", …])` on Windows hits `System32\bash.exe` first — `CreateProcess` searches System32 before PATH, so no PATH trick works), and Windows backslash/drive paths injected into bash scripts are normalized to MSYS form (`C:\x` → `/c/x`) and quoted. `TestDispatchOwnershipChecks` stays skipped on Windows (POSIX ownership/world-writable bits don't map to NTFS).
